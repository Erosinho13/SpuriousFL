...
class StackedMNIST(MNIST):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, num_images: int = 60000,
        ...
        self.map_targets()
        for v in self.target_to_original.values():
            random.shuffle(v)
        ...
        if dirichlet_groups_alpha > 0:
            proportions = np.random.dirichlet(np.repeat(dirichlet_groups_alpha, 6))
        ...
        for i in range(self.num_images):
            if dirichlet_groups_alpha == 0:
                ...
                if train:
                    original_label = random.choices(self.target_to_original[id_label], proportions, k=1)[0]
                else:
                    original_label = self.target_to_original[id_label][self.last_test_original_id[id_label]]
                    self.last_test_original_id[id_label] += 1
                    if self.last_test_original_id[id_label] == 6:
                        self.last_test_original_id[id_label] = 0

            original_label_list = [int(i) for i in list(str(original_label).zfill(3))]
            img_id_list = []
            ...
            id_label += 1
            if id_label == len(self.target_to_original.keys()):  # = Bin(10, 3) = 120
                id_label = 0
    ...
    def map_targets(self):
        ...
        for i in range(1000):
            ...
            if str_unordered_target not in set(self.target_to_unordered.values()):
