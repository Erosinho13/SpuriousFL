...
def acceptance_class_rate(predictions, class_label, attributes, attribute):
     return np.mean(predictions[attributes==attribute]==class_label)
...
def compute_all_metrics(orig_labels, pred_labels, protected_attributes_dict):
    ...
    AR_priv = acceptance_class_rate(pred_labels, fav_lab, protected_attributes, priv_attr)
    AR_unpriv = acceptance_class_rate(pred_labels, fav_lab, protected_attributes, unpriv_attr)
    Dem_Par = AR_priv - AR_unpriv # or np.abs(AR_priv - AR_unpriv)
    ...
    return {'f1': f1, 'macrof1': macrof1, 'macrof1_ind':macrof1_ind, 'ba': ba, 'EOp': EOp, 'Diff_EOds': Diff_EOds, 'Abs_EOds': Abs_EOds, 'AR_priv': AR_priv, 'AR_unpriv': AR_unpriv, 'Dem_Par':Dem_Par}
